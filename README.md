...
- **Read-only by default.** Mutating members (`Insert`/`Modify`/`Delete`/`CommitChanges`/`SetUserProperty`/`Operation.*`) are rejected unless the call sets `allowMutations=true` **and** the server was started with `TEKLA_MCP_ALLOW_SCRIPT_WRITES=1`.
...
This is a pragmatic barrier for well-behaved agents, not a security sandbox — the script runs with the server's privileges. Enable `TEKLA_MCP_ALLOW_SCRIPT_WRITES` only when you want scripted writes, and prefer the dedicated write tools (preview-by-default) otherwise.
