...
- **Anti-GAC binding redirects restored** after a live repro of issue #7's failure mode: a host with stale Tekla 2021 in the GAC bound it silently against a running Tekla 2023 — scripts worked (they reference the running Tekla's DLLs) while every dedicated tool failed on the remoting-channel version. The 2999.9.9.9 redirects are safe with the byte-loading resolver (single AssemblyResolve hop; worst case is a bind error, not the old StackOverflow) and now also cover `Tekla.Structures.Datatype`/`Tekla.Structures.Plugins` to prevent mixed-version loads.
